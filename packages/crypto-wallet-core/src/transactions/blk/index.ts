...
export class BLKTxProvider {
  ...
  create({ recipients, utxos = [], change, feeRate, fee, isSweep, replaceByFee, lockUntilDate, lockUntilBlock }) {
    ...
    const blkUtxos = filteredUtxos.map(utxo => {
      const blkUtxo = Object.assign({}, utxo, {
        amount: utxo.value / 1e8,
        txid: utxo.mintTxid,
        outputIndex: utxo.mintIndex
      });
      return new this.lib.Transaction.UnspentOutput(blkUtxo);
    });
    let tx = new this.lib.Transaction().from(blkUtxos);
    if (fee) {
      tx.fee(fee);
    }
    if (feeRate) {
